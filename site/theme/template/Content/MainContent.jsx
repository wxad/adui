...
export default class MainContent extends React.Component {
	render () {
		...
		const propsStartIndex = contentChildren.findIndex(node =>
			JsonML.getTagName(node) === 'h2' &&
			/^Props/.test(JsonML.getChildren(node)[0])
    )
    
    const alertStartIndex = contentChildren.findIndex(node =>
			JsonML.getTagName(node) === 'h2' &&
    ...
    // ComponentDoc key 要解决的问题是：让组件在 url 变化时，重置 state
		return (
			<ComponentDoc key={this.props.params.children} doc={localizedPageData} demos={demos} {...this.props} />
		)
	}
}